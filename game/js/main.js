...
var MapObject = function( pos, rotation, appearance ){
  this.meshReference;
  this.pos = {
    x: pos.x,
    y: pos.y,
  ...
  console.log(typeof(appearance.texture));

  if ( typeof(appearance.texture) == "object"  ){
    this.texture = [];
    for (var i = 0; i < appearance.texture.length; i++) {
      let textureUrl = "textures/"+appearance.texture[i];
      this.texture[i] = new THREE.TextureLoader().load( textureUrl );
      if ( typeof(appearance.repeat) == "object" && appearance.repeat[i] ) {
        this.texture[i].wrapS = THREE.RepeatWrapping;
        this.texture[i].wrapT = THREE.RepeatWrapping;
        this.texture[i].repeat.set( appearance.repeat[i], appearance.repeat[i] );
      } else if ( appearance.repeat ) {
        this.texture[i].wrapS = THREE.RepeatWrapping;
        this.texture[i].wrapT = THREE.RepeatWrapping;
        this.texture[i].repeat.set( appearance.repeat, appearance.repeat );
      }
    }
  } else {
    let textureUrl = "textures/"+appearance.texture;
  ...
  loader.load( 'meshes/scenery/'+appearance.mesh+'.json?'+Math.random(), function ( geometry ) {
    let material;
    if ( Array.isArray(that.texture) ) {
      material = [];
      for (var i = 0; i < that.texture.length; i++) {
        material[i] = new THREE.MeshLambertMaterial( {map: that.texture[i]} );
      }
    } else {
      material = new THREE.MeshLambertMaterial( {map: that.texture} );
    }

    console.log(material);

    that.meshReference = new THREE.Mesh( geometry, material );
    ...
    that.meshReference.rotation.z = rotation;
...
var wallAppearance = {
  "mesh": "wall-plaster",
  "texture": "plaster-wall.png"
}

objects.push( new MapObject( {"x": 2.5, "y": -2, "z": 0}, 0, wallAppearance));
objects.push( new MapObject( {"x": 2.5, "y": -3, "z": 0}, 0, wallAppearance));
objects.push( new MapObject( {"x": 2.5, "y": -4, "z": 0}, 0, wallAppearance));
objects.push( new MapObject( {"x": 3, "y": -1.5, "z": 0}, Math.PI/2, wallAppearance));
objects.push( new MapObject( {"x": 4, "y": -1.5, "z": 0}, Math.PI/2, wallAppearance));
objects.push( new MapObject( {"x": 5, "y": -1.5, "z": 0}, Math.PI/2, wallAppearance));
objects.push( new MapObject( {"x": 3, "y": -4.5, "z": 0}, Math.PI/2, wallAppearance));
objects.push( new MapObject( {"x": 4, "y": -4.5, "z": 0}, Math.PI/2, wallAppearance));
objects.push( new MapObject( {"x": 5, "y": -4.5, "z": 0}, Math.PI/2, wallAppearance));
objects.push( new MapObject( {"x": 5.5, "y": -2, "z": 0}, 0, wallAppearance));
// objects.push( new MapObject( {"x": 5.5, "y": -3, "z": 0}, 0, wallAppearance));
objects.push( new MapObject( {"x": 5.5, "y": -4, "z": 0}, 0, wallAppearance));
...
function init() {
  ...
  socket.on('map', function (data){
    ...
    for (mapObject of dataObjects) {
      ...
      objects.push( new MapObject( properPosition, mapObject.rotation, mapObject.appearance ) );
    }
  });
